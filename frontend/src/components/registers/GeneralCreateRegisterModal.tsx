...
import { CalendarDaysIcon, ClockIcon, UserGroupIcon, MapPinIcon } from '@heroicons/react/24/outline';
...
interface GeneralCreateRegisterModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRegisterCreated: () => void;
}

const GeneralCreateRegisterModal: React.FC<GeneralCreateRegisterModalProps> = ({
  isOpen,
  onClose,
  onRegisterCreated
}) => {
  ...
  // Get sessions from the selected activity
  const getSessionsForActivity = (activityId: string) => {
    const activity = activities.find(a => a.id === activityId);
    if (!activity?.sessions) return [];
    
    // Flatten session blocks into individual sessions
      ...
      session.sessionBlocks.forEach(block => {
        if (block.isActive) {
          sessions.push({
            ...
            capacity: 20, // Default capacity, could be made configurable
            bookingsCount: block._count.bookings,
            activity: {
              id: activity.id,
              title: activity.title,
              type: activity.type,
              venue: activity.venue || { id: '', name: 'No venue' }
            }
          });
        }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.activityId || !formData.sessionId || !formData.date) {
      toast.error('Please select an activity, session, and date');
      return;
    }
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Create Register">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {loading ? (
            <div className="p-3 border border-gray-300 rounded-md bg-gray-50">
              <div className="animate-pulse text-sm text-gray-500">Loading activities...</div>
            </div>
          ) : activities.length === 0 ? (
            <div className="p-3 border border-gray-300 rounded-md bg-gray-50">
        ...
        <div>
          ...
          {!formData.activityId ? (
            <div className="p-3 border border-gray-300 rounded-md bg-gray-50">
              <div className="text-sm text-gray-500">Please select an activity first</div>
            </div>
          ) : (() => {
            const sessions = getSessionsForActivity(formData.activityId);
        ...
        {/* Selected Activity/Session Info */}
        {formData.activityId && formData.sessionId && (
          <div className="bg-gray-50 p-4 rounded-lg">
            <h4 className="font-medium text-gray-900 mb-2">Register Details</h4>
            {(() => {
              ...
              if (!selectedActivity || !selectedSession) return null;
              
              return (
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex items-center">
                    <CalendarDaysIcon className="h-4 w-4 mr-2" />
                    <span><strong>{selectedActivity.title}</strong></span>
                  </div>
                  <div className="flex items-center">
                    <MapPinIcon className="h-4 w-4 mr-2" />
                    <span>{selectedActivity.venue?.name || 'No venue'}</span>
                  </div>
                  <div className="flex items-center">
                    <ClockIcon className="h-4 w-4 mr-2" />
                    <span>{formatDate(selectedSession.date)} - {formatTime(selectedSession.startTime)} to {formatTime(selectedSession.endTime)}</span>
                  </div>
                  <div className="flex items-center">
        ...
        <div className="flex space-x-3 pt-4">
          ...
          <Button
            type="submit"
            disabled={submitting || !formData.activityId || !formData.sessionId || !formData.date}
            className="flex-1"
          >
...
export default GeneralCreateRegisterModal;
